<?php
namespace MikadoListing\Lib\Core;
class ListingQuery
{
    ...
    private function setQueryResults() {
        ...
        if (isset($this->location_object['dist']) && isset($this->location_object['lat']) && isset($this->location_object['long'])) {
            ...
            $geo_location_answer = mkdf_listing_job_check_distance($lat, $long, $dist, $posts_to_check);
...
class ListingRating
{
    ...
    private function getRateNumber() {
        return get_post_meta($this->post_id, 'mkdf_listing_job_post_rating_number', true);
    }

    private function setRateNumber($count) {
        update_post_meta($this->post_id, 'mkdf_listing_job_post_rating_number', $count);
    }
    ...
    private function getRateCount() {
        return get_post_meta($this->post_id, 'mkdf_listing_job_post_rating_value', true);
    }

    private function setRateCount($value) {
        update_post_meta($this->post_id, 'mkdf_listing_job_post_rating_value', $value);
    }
    ...
    public function getRatingHtml($show_number = true) {
        ...
        ?>
        <div class="mkdf-listing-rating-holder">
            ...
            <div class="mkdf-listing-rating-stars-holder">
...
class ListingViews
{
    ...
    public function __construct($post_id, $user_address = '') {
        ...
        $this->cookie_name = 'mkdf_listing_job_single_id_' . $this->post_id;
...
//            setcookie($this->cookie_name, $this->cookie_name, time() * 20, '/');
    ...
    public function getViewCount() {
        return get_post_meta($this->post_id, 'mkdf_listing_job_post_view_count', true);
    }

    private function setViewCount($value) {
        update_post_meta($this->post_id, 'mkdf_listing_job_post_view_count', $value);
    }
...
class ListingArticle
{
        ...
        if (is_array($taxes) && count($taxes)) {
            foreach ($taxes as $tax) {
                ...
                $tax_array[] = array(
                    ...
                    'icon_html' => mkdf_listing_job_get_listing_taxonomy_icon_html($tax->term_id)
                );
    ...
    public function getTaxHtml($taxonomy, $custom_css_class = '') {
        ...
        $type_background = $this->getPostMeta('_listing_type_background');
        $type_style = '';

        if($type_background !== '') {
            $type_style = 'background-color: ' . esc_attr($type_background) . ';';
        }
        ...
        if (count($taxes)) {
            ...
            foreach ($taxes as $tax) {
                ...
                if($taxonomy === 'job_listing_type') {
                    $html .= '<a href=" ' . esc_url($tax['link']) . ' " ' . staffscout_mikado_get_inline_style($type_style) . '>';
                } else {
    ...
    public function getListingAverageRating($show_number = true) {
    ...
    public function getAddressIconHtml($show_icon = true, $show_label = true) {
        ...
        $params_address = mkdf_listing_job_get_address_params($this->post_id);
    ...
    public function getPriceRateHtml() {
        ...
        $price_rate = $this->getPostMeta('_listing_rate');

        if (($price_rate && $price_rate !== '')) {

            $price_rate_html .= '<div class="mkdf-ls-price-rate-holder">';
            if ($price_rate && $price_rate !== '') {
                $price_rate_html .= '<span class="mkdf-ls-price-rate-amount">';
    ...
    public function getExpireDateHtml() {
        ...
        if (($expire_date && $expire_date !== '')) {
            ...
            $expire_date_html .= '<div class="mkdf-ls-expire-date-holder">';
            if ($expire_date && $expire_date !== '') {
                $expire_date_html .= '<span class="mkdf-ls-expire-date-inner">';
...
class ListingTitleGlobalVar
{
    ...
    public function generateGlobalVar() {
        ...
        $query_results = mkdf_listing_job_get_listing_query_results($params);
        ...
        $listing_posts = apply_filters('mkdf_listing_job_filter_js_listing_variables', $listing_posts);

        wp_localize_script('staffscout_mikado_modules', 'mkdfListingTitles', array(
...
class ListingRelatedPosts
{
    ...
    private function getPosts($tax_key, $post_taxes) {
        ...
        if ($this->post_type === 'job_listing') {
            ...
            return mkdf_listing_job_get_listing_query_results($params);
        } else {
            $args = array(
                'post_not_in' => array($this->post_id),
                'order'       => 'DESC',
