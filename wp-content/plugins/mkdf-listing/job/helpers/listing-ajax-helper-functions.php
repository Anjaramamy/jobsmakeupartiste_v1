...
if (!function_exists('mkdf_listing_job_type_get_custom_fields')) {

    function mkdf_listing_job_type_get_custom_fields() {
        $type_id = $post_id = '';
        $html = '';
    ...
    add_action('wp_ajax_nopriv_mkdf_listing_job_type_get_custom_fields', 'mkdf_listing_job_type_get_custom_fields');
    add_action('wp_ajax_mkdf_listing_job_type_get_custom_fields', 'mkdf_listing_job_type_get_custom_fields');
}


if (!function_exists('mkdf_listing_job_get_archive_search_response')) {

    function mkdf_listing_job_get_archive_search_response() {
            ...
            if ($query_results->have_posts()) {
                while ($query_results->have_posts()) {
                    ...
                    $params = array(
                        ...
                        'cat_html'     => $article->getTaxHtml('job_listing_category', 'mkdf-listing-cat-wrapper'),
                        'title'        => $title,
                        'img_style'    => $img_style,
                        'excerpt'      => $excerpt,
                    );
    ...
    add_action('wp_ajax_nopriv_mkdf_listing_job_get_archive_search_response', 'mkdf_listing_job_get_archive_search_response');
    add_action('wp_ajax_mkdf_listing_job_get_archive_search_response', 'mkdf_listing_job_get_archive_search_response');
}

if (!function_exists('mkdf_listing_job_get_main_search_response')) {

    function mkdf_listing_job_get_main_search_response() {
        ...
        if (isset($_POST)) {
            ...
            extract($params);
            $href_attr = mkdf_listing_job_build_query_string($keyword, $type, $salary);
    ...
    add_action('wp_ajax_nopriv_mkdf_listing_job_get_main_search_response', 'mkdf_listing_job_get_main_search_response');
    add_action('wp_ajax_mkdf_listing_job_get_main_search_response', 'mkdf_listing_job_get_main_search_response');
}

if (!function_exists('mkdf_listing_job_send_listing_item_enquiry')) {

    function mkdf_listing_job_send_listing_item_enquiry() {
        if (isset($_POST['data'])) {
            ...
            if (wp_verify_nonce($nonce, 'mkdf_listing_job_validate_listing_item_enquiry')) {
    ...
    add_action('wp_ajax_nopriv_mkdf_listing_job_send_listing_item_enquiry', 'mkdf_listing_job_send_listing_item_enquiry');
    add_action('wp_ajax_mkdf_listing_job_send_listing_item_enquiry', 'mkdf_listing_job_send_listing_item_enquiry');
}