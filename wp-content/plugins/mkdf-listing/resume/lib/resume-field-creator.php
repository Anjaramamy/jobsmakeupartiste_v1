<?php
namespace MikadoResume\Lib\FieldCreator;

class ResumeFieldCreator{
	...
	public function __construct($id = '', $type = '', $label = '', $options = array(), $required = false,$placeholder = '',$priority = '',$description = '',$multiple = false, $front_end_field = true , $back_end_field = true){
		...
		if($this->front_end_field){
			add_filter('submit_resume_form_fields', array($this, 'create_front_end_field'));
		}
		if($this->back_end_field){
			add_filter('resume_manager_resume_fields', array($this, 'create_back_end_field'));
		}
	}

	public function create_front_end_field($fields){

		$fields['resume_fields'][$this->id] = array(
			'label'       => $this->label,
