...
/*
   Class: StaffScoutMikadoClassFramework
...
class StaffScoutMikadoClassFramework {
	...
	private function __construct() {
		$this->mkdOptions         = StaffScoutMikadoClassOptions::get_instance();
		$this->mkdMetaBoxes       = StaffScoutMikadoClassMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = StaffScoutMikadoClassTaxonomyOptions::get_instance();
	}
	...
	public function setSkin( StaffScoutMikadoClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class StaffScoutMikadoClassSkinManager
 *
...
class StaffScoutMikadoClassSkinManager {
	...
	/**
	 * @see StaffScoutMikadoClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = 'mikado' ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'staffscout' );
...
/**
 * Class StaffScoutMikadoClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class StaffScoutMikadoClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( staffscout_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo staffscout_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'staffscout' ) ?>';
		</script>
	<?php }
...
/*
   Class: StaffScoutMikadoClassOptions
...
class StaffScoutMikadoClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $theme_name_php_global_options ) ) {
			return $theme_name_php_global_options[ $key ];
...
/*
   Class: StaffScoutMikadoClassAdminPage
   A class that initializes Mikado Admin Page
*/
class StaffScoutMikadoClassAdminPage implements iStaffScoutMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new StaffScoutMikadoClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iStaffScoutMikadoInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: StaffScoutMikadoClassMetaBoxes
...
/*
   Class: StaffScoutMikadoClassMetaBox
   A class that initializes Mikado Meta Box
*/
class StaffScoutMikadoClassMetaBox implements iStaffScoutMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $theme_name_php_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: StaffScoutMikadoClassTaxonomyOptions
   A class that initializes StaffScoutMikadoClass Taxonomy Options
*/
class StaffScoutMikadoClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: StaffScoutMikadoClassTaxonomyOption
   A class that initializes StaffScoutMikadoClass Taxonomy Option
*/
class StaffScoutMikadoClassTaxonomyOption implements iStaffScoutMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new StaffScoutMikadoClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
if ( ! function_exists( 'staffscout_mikado_init_framework_variable' ) ) {
	function staffscout_mikado_init_framework_variable() {
		global $theme_name_php_global_Framework;
		
		$theme_name_php_global_Framework = StaffScoutMikadoClassFramework::get_instance();
		$mkdSkinManager                 = new StaffScoutMikadoClassSkinManager();
		$theme_name_php_global_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'staffscout_mikado_before_options_map', 'staffscout_mikado_init_framework_variable' );
}
?>