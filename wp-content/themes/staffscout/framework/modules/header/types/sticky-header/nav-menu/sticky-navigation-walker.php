...
if (!class_exists('StaffScoutMikadoClassStickyNavigationWalker')) {
	class StaffScoutMikadoClassStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if($depth==0){
                if($item->type_menu == "wide"){
                    $menu_type = " wide";
                } else if ( $item->type_menu === 'wide-2-columns' ) {
                    $menu_type = " wide mkdf-menu-2-columns";
                } else {
                    $menu_type = " narrow";
                }
            }
			...
			// build html
			$output .= $indent . '<li id="sticky-nav-menu-item-'. $item->ID . '" class="' . $class_names . ' '. $active . $sub . $menu_type .'">';
